...
class Bank(commands.Cog):
    ...
    async def deposit_money(self, ctx, amount: str = None):
        ...
        if amount is None:
            ...
            deposit_limit = self.get_card_limit(card_tier, is_premium)
            ...
            embed.add_field(
                name="Deposit Limit",
                value=f"`{limit_text}` <:mora:1437958309255577681>",
                inline=True
            )
    ...
    async def bank_card(self, ctx, action: str = None):
        ...
        if action is None:
            ...
            if next_tier < 2:
                ...
                embed.set_footer(text=f"Next tier: {next_next['icon']} {next_next['name']} - {next_next['cost']:,} mora")
            else:
