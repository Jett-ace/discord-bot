...
class Moderation(commands.Cog):
    ...
    async def grant(self, ctx, item: str, amount: int, member: str = None):
        ...
        Items: common, exquisite, precious, luxurious, mora, tidecoins, fate, 
               lucky_dice, golden_chip, xp_booster, bankers_key, streak_shield, hot_streak, 
               rigged_deck, plasma_canon, special_crate, bank_upgrade, shotgun, thiefpack, 
               guarddog, fence, lock
        ...
        # Check for random chest (the only chest type we grant)
        if item_lower == "random":
            try:
                async with aiosqlite.connect(DB_PATH) as db:
                    await db.execute(
                        "INSERT INTO inventory (user_id, item_id, quantity) VALUES (?, 'random', ?) "
                        "ON CONFLICT(user_id, item_id) DO UPDATE SET quantity = quantity + ?",
                        (target_member.id, amount, amount)
                    )
                    await db.commit()
                await ctx.send(
                    f"<a:Y_:1437951897989730305> Gave <:random:1437977751520018452> {amount} random chest(s) to {target_member.display_name}."
                )
            except Exception as e:
                print(f"Failed to grant random chest(s): {e}")
                await ctx.send(f"Failed to grant random chests.")
            return

        if item_lower in ("chest", "chests"):
            await ctx.send("❌ Please use `random` for chest type. Old chest types are disabled.")
            return

        if item_lower in ("mora", "gold"):
            data["mora"] += amount
            await update_user_data(target_member.id, mora=data["mora"])
            await ctx.send(f"Gave {amount:,} 💰 to {target_member.display_name}.")
            return
        ...
        if item_lower in ("tide_coins", "tidecoins", "tide"):
            ...
            await ctx.send(
                f"Gave {amount} <:mora:1437480155952975943> Tide Coins to {target_member.display_name}."
            )
            return
        ...
        if item_lower in ("fate", "fates"):
            ...
            await ctx.send(
                f"Gave {amount} <:fate:1437488656767254528> Intertwined Fate(s) to {target_member.display_name}."
            )
            return

        if item_lower in ("hydro_essence", "hydroessence", "essence"):
            await add_user_item(target_member.id, "hydro_essence", amount)
            await ctx.send(
                f"Gave {amount} <:essence:1437463601479942385> Hydro Essence to {target_member.display_name}."
            )
            return

        if item_lower in ("hydro_crystal", "hydrocrystal", "crystal"):
            await add_user_item(target_member.id, "hydro_crystal", amount)
            await ctx.send(
                f"Gave {amount} <:crystal:1437458982989205624> Hydro Crystal to {target_member.display_name}."
            )
            return

        if item_lower in ("rod_shard", "rodshard", "shard"):
            await add_user_item(target_member.id, "rod_shard", amount)
            await ctx.send(f"Gave {amount} 🔧 Rod Shard(s) to {target_member.display_name}.")
            return

        if item_lower in ("fish_bait", "fishbait", "bait"):
            await add_user_item(target_member.id, "fish_bait", amount)
            await ctx.send(f"Gave {amount} 🪱 Fish Bait to {target_member.display_name}.")
            return

        # Black Market Items
        blackmarket_items = {
            "lucky_dice": ("<:dice:1457965149137670186>", "Lucky Dice"),
            "golden_chip": ("<:goldenchip:1457964285207646264>", "Golden Chip"),
            "xp_booster": ("<:exp:1437553839359397928>", "XP Booster"),
            "bankers_key": ("<a:bankerskey:1457962936076075049>", "Banker's Key"),
            "streak_shield": ("<a:shield:1457967376799629324>", "Streak Shield"),
            "hot_streak": ("<:streak:1457966635838214247>", "Hot Streak Card"),
            "rigged_deck": ("<a:deck:1457965675082551306>", "Rigged Deck"),
            "plasma_canon": ("<:plasmacanon:1457975521521434624>", "Plasma Canon"),
            "special_crate": ("<a:crate:1457969509770985492>", "Special Crate"),
            "bank_upgrade": ("<:upgrade:1457983244682268695>", "Bank Upgrade"),
            "shotgun": ("🔫", "Shotgun"),
            "thiefpack": ("🎒", "Thief Pack"),
            "guarddog": ("🐕", "Guard Dog"),
            "fence": ("<:fench:1458002114260242454>", "Spiky Fence"),
            "lock": ("🔒", "Lock")
        }
        
        if item_lower in blackmarket_items:
            emoji, name = blackmarket_items[item_lower]
            async with aiosqlite.connect(DB_PATH) as db:
                # Check if user already has this item
                async with db.execute(
                    "SELECT quantity FROM inventory WHERE user_id = ? AND item_id = ?",
                    (target_member.id, item_lower)
                ) as cursor:
                ...
                if result:
                    ...
                    await db.execute(
                        ...
                        (amount, target_member.id, item_lower)
                    )
                else:
                    # Insert new
                    await db.execute(
                        "INSERT INTO inventory (user_id, item_id, quantity) VALUES (?, ?, ?)",
                        (target_member.id, item_lower, amount)
                    )
                await db.commit()
            
            await ctx.send(f"Gave {amount} {emoji} **{name}** to {target_member.display_name}.")
            return

        await ctx.send(f"Unknown item: {item}. Check ggrant help for available items.")
