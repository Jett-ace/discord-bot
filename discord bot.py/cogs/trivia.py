...
class Trivia(commands.Cog):
    ...
    async def skip_question(self, ctx):
        ...
        if is_player1:
            ...
            skips_left = game.player1_skips
            current_idx = game.player1_current
        else:
            ...
            skips_left = game.player2_skips
            current_idx = game.player2_current
        
        # Replace the skipped question with a new random question
        ...
        # If we have available questions, replace the current one
        if available_questions:
            new_question = random.choice(available_questions)
            game.questions[current_idx] = new_question
        # If no more questions available (all 70 used), keep current question but increment
        else:
            if is_player1:
        ...
        # Clear revealed letters for the new question
        if is_player1:
            game.player1_revealed_letters.clear()
        else:
            game.player2_revealed_letters.clear()
        
        await ctx.send(f"🔄 Question skipped! New question provided. **{skips_left}** skips remaining.")
        
        # Send next question (or the replacement question)
