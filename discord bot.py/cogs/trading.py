...
from utils.database import (
    ...
    add_user_item, get_user_item_count, get_chest_inventory,
    change_chest_type_count, ensure_user_db
)
...
class Trading(commands.Cog):
    ...
    async def trade(self, ctx, target: discord.Member = None, *, trade_details: str = None):
        """Send a trade offer to another user.
        
        Usage: !trade @user (my_items) (their_items)
        ...
        Example: !trade @user (common_5, mora_10000) (precious_2, tidecoins_5000)
        ...
        if target is None or trade_details is None:
            ...
            embed.description = "`!trade @user (myitems_amount) (theiritems_amount)`"
        ...
        try:
            ...
            # Parse both offers
            my_offer = await self.parse_offer(ctx.author.id, my_items_str, ctx.author.display_name)
            their_offer = await self.parse_offer(target.id, their_items_str, target.display_name, validate=False)
            ...
            # Create embed
            embed = discord.Embed(
                title=f"Trade Offer from {ctx.author.display_name}",
                color=0xe67e22
            )
    ...
    async def parse_offer(self, user_id, items_str, username, validate=True):
        ...
        for item in items_list:
            ...
            if item_lower == 'mora':
                ...
                if validate:
                    ...
                    if data['mora'] < offer['mora']:
                        await self.bot.get_channel(self.bot.guilds[0].text_channels[0].id).send(
                            f"{username} doesn't have {offer['mora']:,} Mora."
                        )
                        return None
                        
            elif item_lower in ['tidecoins', 'tidecoin', 'tide_coins', 'tide_coin', 'coins', 'coin']:
                offer['dust'] += amount
                if validate:
                    data = await get_user_data(user_id)
                    if data['dust'] < offer['dust']:
                        await self.bot.get_channel(self.bot.guilds[0].text_channels[0].id).send(
                            f"{username} doesn't have {offer['dust']:,} Tide Coins."
                        )
                        return None
            ...
            # Check for chests
                ...
                offer['chests'][chest_type] = offer['chests'].get(chest_type, 0) + amount
                if validate:
            ...
            # Check for items
                ...
                offer['items']['exp_bottle'] = offer['items'].get('exp_bottle', 0) + amount
                if validate:
                ...
                offer['items']['hydro_essence'] = offer['items'].get('hydro_essence', 0) + amount
                if validate:
                ...
                offer['items']['hydro_crystal'] = offer['items'].get('hydro_crystal', 0) + amount
                if validate:
    ...
    async def execute_trade(self, trade_data):
        ...
        try:
            ...
            valid_from = await self.parse_offer(from_user.id, self.offer_to_string(from_offer), from_user.display_name, validate=True)
            valid_to = await self.parse_offer(to_user.id, self.offer_to_string(to_offer), to_user.display_name, validate=True)
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                new_from_mora = from_data['mora'] - from_offer['mora'] + to_offer['mora']
                new_from_dust = from_data['dust'] - from_offer['dust'] + to_offer['dust']
                new_to_mora = to_data['mora'] - to_offer['mora'] + from_offer['mora']
                new_to_dust = to_data['dust'] - to_offer['dust'] + from_offer['dust']
                
                await db.execute("UPDATE users SET mora=?, dust=? WHERE user_id=?",
...
class TradeOfferView(discord.ui.View):
    ...
    @discord.ui.button(label="Accept", style=discord.ButtonStyle.success, emoji="<a:Check:1437951818452832318>")
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.trade_id not in self.cog.pending_trades:
            await interaction.response.send_message("This trade has expired.", ephemeral=True)
            return
        ...
        if success:
            embed = discord.Embed(title="<a:Check:1437951818452832318> Trade Completed!", description=message, color=0x2ecc71)
            embed.add_field(
            ...
            embed = discord.Embed(title="<a:X_:1437951830393884788> Trade Failed", description=message, color=0xe74c3c)
        
        await interaction.response.edit_message(embed=embed, view=None)

    @discord.ui.button(label="Decline", style=discord.ButtonStyle.danger, emoji="<a:X_:1437951830393884788>")
