...
from utils.database import (
    get_user_data, update_user_data, get_user_pulls,
    add_chest_with_type, get_account_level, add_account_exp,
    award_achievement, _exp_required_for_level, add_fish_caught,
    get_fish_count_by_rarity, get_total_fish_caught, add_fish_pet,
    add_user_item, get_user_item_count, get_user_fish_pets, level_up_fish_pet
)
from utils.constants import regions, FISHING_MIN_LEVEL, EXP_TUNING, fish_pool, fish_rarity_weights
...
class Fishing(commands.Cog):
    ...
    async def fish(self, ctx, *args):
        ...
        if not args:
            ...
            if lvl < FISHING_MIN_LEVEL:
                ...
                try:
                    ...
                    for l in range(lvl + 1, FISHING_MIN_LEVEL):
                        total_exp += await _exp_required_for_level(l)
                    embed.add_field(name="EXP Needed", value=f"{total_exp:,} EXP to reach level {FISHING_MIN_LEVEL}", inline=False)
                except Exception:
                    pass
                embed.set_footer(text="Gain EXP by wishing, opening chests, and completing dispatches!")
            else:
                embed.add_field(name="Usage", value="`!fish <character> <region>`", inline=False)
                embed.add_field(name="Examples", value="`!fish Amber mondstadt`\n`!fish Hu Tao fontaine`", inline=False)
                embed.set_footer(text="Use `!map` to see available regions.")
        ...
        if region is None:
            await ctx.send("Usage: `!fish <character> <region>`. Example: `!fish Amber mondstadt`")
            return
        ...
        if ctx.author.id in self._fish_cooldowns:
            ...
            if time_since < last_cooldown:
                ...
                await ctx.send(f"You can't fish right now. Wait {time_msg} before you can fish again.")
                return
        ...
        if lvl < region_level:
            ...
            try:
                ...
                for l in range(lvl + 1, region_level):
        ...
        if character:
            ...
            for p in pulls:
                ...
                if name == character.strip().lower():
                    ...
                    break
            if not found:
                await ctx.send("You don't own that character to use for a fishing bonus.")
                return
        ...
        # Fish catch chance: 15% base, 25% with 5★ same-region bonus
        fish_catch_chance = 0.25 if (same_region_bonus and found and found[1] == '5★') else 0.15
        ...
        if random.random() < fish_catch_chance:
            ...
            for rarity, weight in fish_rarity_weights.items():
                cumulative += weight
                if rand < cumulative:
        ...
        if random.random() < exp_bottle_chance:
            try:
                ...
                await add_user_item(ctx.author.id, 'exp_bottle', 1)
        ...
        fishing_exp = 0
        try:
            fishing_exp = int(EXP_TUNING.get('dispatch_per_region_level', 10) * max(1, region_level))
            if fishing_exp > 0:
        ...
        if caught_fish:
            ...
            rewards_lines.append(f"{caught_fish['icon']} {caught_fish['name']} ({rarity_text})")
        
        if awarded_chest > 0:
            chest_text = f"{awarded_chest} **Common** Chest" if awarded_chest == 1 else f"{awarded_chest} **Common** Chests"
            rewards_lines.append(f"<:cajitadelexplorador:1437473147833286676> {chest_text}")
        if awarded_exp_bottles > 0:
            rewards_lines.append(f"EXP Bottle")
        if awarded_fate:
            rewards_lines.append(f"Intertwined Fate")
        if awarded_hydro_essence > 0:
            rewards_lines.append(f"<:essence:1437463601479942385> {awarded_hydro_essence} Hydro Essence")
        ...
        # Combine bonus info: same-region and EXP as a field
        bonus_parts = []
        if same_region_bonus:
        ...
        try:
            # First fish ever
            await award_achievement(ctx.author.id, 'first_fish', 'First Cast', 'You fished for the first time.')
            
            # Check if caught first fish (not just fished)
            if caught_fish:
                total_fish = await get_total_fish_caught(ctx.author.id)
                if total_fish == 1:
                    await award_achievement(ctx.author.id, 'first_fish_caught', 'Angler', 'You caught your first fish!')
                ...
                if common_count >= 5:
                    await award_achievement(ctx.author.id, 'all_common_fish', 'Common Collector', 'Caught all common fish!')
                if rare_count >= 7:
                    await award_achievement(ctx.author.id, 'all_rare_fish', 'Rare Hunter', 'Caught all rare fish!')
                if mythic_count >= 3:
                    await award_achievement(ctx.author.id, 'all_mythic_fish', 'Mythic Master', 'Caught all mythic fish!')
                
                # Complete encyclopedia achievement
                if common_count >= 5 and rare_count >= 7 and mythic_count >= 3:
                    await award_achievement(ctx.author.id, 'fish_master', 'Fish Encyclopedia', 'Caught every single fish in Teyvat!')
                
                # Milestone achievements
                if total_fish >= 10:
                    await award_achievement(ctx.author.id, 'fish_10', 'Novice Angler', 'Caught 10 fish total.')
                if total_fish >= 50:
                    await award_achievement(ctx.author.id, 'fish_50', 'Experienced Angler', 'Caught 50 fish total.')
                if total_fish >= 100:
                    await award_achievement(ctx.author.id, 'fish_100', 'Master Angler', 'Caught 100 fish total.')
                
                # Special achievement for mythic fish
                if caught_fish['rarity'] == 'Mythic':
                    await award_achievement(ctx.author.id, 'first_mythic', 'Mythic Catch', 'You caught your first mythic fish!')
        except Exception:
            pass
    ...
    async def fishbook(self, ctx):
        ...
        if not pets:
            ...
            embed.set_footer(text="Fish in Fontaine to get Hydro Essence for leveling pets!")
            await send_embed(ctx, embed)
            return
        ...
        embed = discord.Embed(title=f"🐟 {ctx.author.display_name}'s Fish Pets", color=0x3498db)
        ...
        if common_pets:
            ...
            for pet in common_pets[:10]:  # Limit to 10 to avoid embed length issues
                common_lines.append(f"{pet['icon']} **{pet['name']}** - Lv.{pet['level']}")
            if len(common_pets) > 10:
                common_lines.append(f"... and {len(common_pets) - 10} more")
            embed.add_field(name="⭐ Common Pets", value="\n".join(common_lines), inline=False)
        
        # Show rare pets
        if rare_pets:
            rare_lines = []
            for pet in rare_pets[:10]:
                rare_lines.append(f"{pet['icon']} **{pet['name']}** - Lv.{pet['level']}")
            if len(rare_pets) > 10:
                rare_lines.append(f"... and {len(rare_pets) - 10} more")
            embed.add_field(name="⭐⭐ Rare Pets", value="\n".join(rare_lines), inline=False)
        
        # Show mythic pets
        if mythic_pets:
            mythic_lines = []
            for pet in mythic_pets:
                mythic_lines.append(f"{pet['icon']} **{pet['name']}** - Lv.{pet['level']}")
            embed.add_field(name="⭐⭐⭐ Mythic Pets", value="\n".join(mythic_lines), inline=False)
        ...
        try:
            ...
            embed.add_field(
                name="<:essence:1437463601479942385> Materials",
                value=f"Hydro Essence: {essence_count}\nHydro Crystals: {crystal_count} <:crystal:1437458982989205624>",
                inline=False
            )
        except Exception:
            pass
        
        embed.set_footer(text="Use !levelpet <name> to level up a pet • Use !mp <name> to view a pet • Fish in Fontaine for Hydro Essence!")
        await send_embed(ctx, embed)

    @commands.command(name="mp", aliases=["mypet", "petinfo"])
    async def my_pet(self, ctx, *, pet_name: str = None):
        """View detailed information about a specific pet.
        Usage: !mp <pet_name>
        Example: !mp Medaka
        """
        if not pet_name:
            await ctx.send("Please specify a pet name. Example: `!mp Medaka`\nUse `!fishbook` to see all your pets.")
            return
        ...
        if not pets:
            await ctx.send("You don't have any pets yet! Use `!fish` to catch fish.")
            return
        ...
        if not matching_pets:
            await ctx.send(f"You don't have a pet named **{pet_name}**. Use `!fishbook` to see your pets.")
            return
        ...
        embed.add_field(name="Highest Level", value=f"Lv.{max_level_pet['level']}", inline=True)
        ...
        if total_count > 1:
            pet_lines = []
            for i, pet in enumerate(sorted(matching_pets, key=lambda p: p['level'], reverse=True)[:5], 1):
        ...
        embed.set_footer(text=f"Use !levelpet {fish_name} to level up this pet")
        await send_embed(ctx, embed)

    @commands.command(name="craft")
    async def craft(self, ctx, item: str = None, amount: int = 1):
        ...
        if not item or item.lower() not in ['hydro_crystal', 'crystal']:
            ...
            embed.add_field(name="Recipe", value="5 <:essence:1437463601479942385> Hydro Essence → 1 <:crystal:1437458982989205624> Hydro Crystal", inline=False)
            embed.add_field(name="Usage", value="`!craft hydro_crystal <amount>`", inline=False)
            embed.set_footer(text="Hydro Essence can be obtained from fishing or dispatches in Fontaine!")
            await send_embed(ctx, embed)
            return
        ...
        if essence_count < essence_needed:
            await ctx.send(f"You don't have enough Hydro Essence. You need {essence_needed} but only have {essence_count}.")
            return
        ...
        try:
            ...
            embed.add_field(name="Materials Used", value=f"-{essence_needed} <:essence:1437463601479942385> Hydro Essence", inline=False)
            crystal_count = await get_user_item_count(ctx.author.id, 'hydro_crystal')
    ...
    async def level_pet(self, ctx, *, pet_name: str = None):
        ...
        if pet_name is None:
            ...
            embed.add_field(name="Crystal Cost", value="Common: 1 <:crystal:1437458982989205624>\nRare: 2 <:crystal:1437458982989205624>\nMythic: 3 <:crystal:1437458982989205624>", inline=False)
            embed.add_field(name="Usage", value="`!levelpet <pet_name>`\nExample: `!levelpet Medaka`", inline=False)
            embed.set_footer(text="Use !fishbook to see your pets")
            await send_embed(ctx, embed)
            return
        ...
        if crystal_count < crystals_needed:
            await ctx.send(f"You don't have enough Hydro Crystals. You need {crystals_needed} but only have {crystal_count}.")
            return
        ...
        if success:
            ...
            embed = discord.Embed(title="⬆️ Pet Leveled Up!", color=0x2ecc71)
            embed.description = f"{fish_info['icon']} **{fish_name}** leveled up!"
            embed.add_field(name="Level", value=f"{target_pet['level']} → {new_level}", inline=True)
            embed.add_field(name="Rarity", value=rarity, inline=True)
            embed.add_field(name="Crystals Used", value=f"-{crystals_needed} <:crystal:1437458982989205624>", inline=False)
            remaining_crystals = crystal_count - crystals_needed
            embed.set_footer(text=f"You have {remaining_crystals} Hydro Crystal(s) remaining")
            await send_embed(ctx, embed)
        else:
